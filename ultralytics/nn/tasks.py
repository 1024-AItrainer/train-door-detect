...
from ultralytics.train_door_detect import (
    StarNet,
    SPPSA,
    SAG,
    LightFusion,
...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            # Multi-scale backbone (StarNet): same indexing as ultralytics-mgmj ours2
            if hasattr(m, "backbone"):
                x = m(x)
                for _ in range(5 - len(x)):
                    x.insert(0, None)
                for i_idx, xi in enumerate(x):
                    y.append(xi if i_idx in self.save else None)
                x = x[-1]
            else:
                x = m(x)  # run
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect() / DESD()
        if isinstance(m, (Detect, DESD)):  # Detect subclasses + paper DESD head
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        t = m  # keep yaml module name string for logging / type
        m = (
        ...
        # SPPSA / ChannelAlign follow mgmj: width-scaled like Conv
        if m in base_modules or m in {SPPSA, ChannelAlign}:
            c1, c2 = ch[f], args[0]
            ...
            c1 = ch[f]
            args = [*args[1:]]
        ############### Paper modules (logic = mgmj ours2) #############
        elif m is StarNet:
            m = m(*args)
            c2 = m.channel
        elif m is SAG:
            c2 = ch[f[0]]
            args = [ch[f[0]]]
        elif m is LightFusion:
            c2 = ch[f[0]]
            args = [[ch[x] for x in f], ch[f[0]]]
        else:
            c2 = ch[f]

        # Backbone multi-scale indexing identical to ultralytics-mgmj:
        # after StarNet, keep is_backbone=True so later layers use i+4 (yaml #5,#6,...,#23).
        ...
        if verbose:
            LOGGER.info(f"{i:>3}{f!s:>20}{n_:>3}{m_.np:10.0f}  {str(t):<45}{args!s:<30}")  # print
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, DESD)):
                return "detect"
